...
- キー未設定 / 取得失敗時は、**実ダイヤ準拠の擬似運行**へ自動フォールバック。
  実際の**時間帯別の運転間隔**（朝夕ラッシュは増発・日中は約5分・深夜は減便）と
  **始発04:26〜終電01:20**、**平日/土休日**の違いまで再現し、実時刻に連動する（実データではない）。
  列車は大崎の車庫から実間隔で出入りする。`ODPT_KEY` を設定すると自動で実運行（LIVE）へ切り替わる。
